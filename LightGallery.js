...
class LightGallery{
  ...
  init(){
    ...
    this.thumbs = this.container.getElementsByTagName('a');
    this.panes = [];
  ...
  getStyles(){
    ...
    this.styles.innerHTML += '.light-gallery-box { position: fixed; display: flex; top: 0; left: 0; height: 100vh; background: rgba(0,0,0,0.8); }';
    ...
    this.styles.innerHTML += '.light-gallery-box-pane { height: 100%; width: 100vw; display: flex; justify-content: center; align-items: center; }';
    this.styles.innerHTML += '.light-gallery-box-pane figure { overflow: hidden }';
  ...
  bindEvents(){
    ...
    //BIND BOX
    this.box.addEventListener( 'click', function(){ self.setUnactive() } );

    this.prev.addEventListener('click', function(){ self.prevPane() });
    this.next.addEventListener('click', function(){ self.nextPane() });
